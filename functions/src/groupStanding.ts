...
export default functions.https.onRequest((req, res) => {
  ...
  return setting.sportmonksApi.get(setting.groups, { id: 892 })
  .then(({ data }:{data: Group[]}) => {
    data.forEach(group => {
      const groupStandingsRef = admin.firestore().collection('groupStandings').doc(group.name.replace(/\s/g, ''))
      ...
      table.forEach(team => {
        const teamRef = admin.firestore().doc('teams/'+team.team_name.toLocaleLowerCase().replace(/\s/g, '-'))
